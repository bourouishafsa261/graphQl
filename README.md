...
query {
  books(page: 3, size: 5) {
    ...
    pageInfo { pageNumber pageSize itemsLeft totalItems hasMore }
  }
}
...
```graphql
query {
  books(publicationYear: 2020) {
    list { title publicationYear author { name } }
    pageInfo { totalItems itemsLeft hasMore }
  }
}
...
query {
  books(language: "English") {
    list { title language author { name } }
    pageInfo { totalItems itemsLeft hasMore }
  }
}
```
...
```graphql
query {
  books(idCategory: 1, recursive: false) {
    list { 
      title 
      category 
      { categoryName } 
...
```graphql
query {
  books(idCategory: 1, recursive: true) {
    list { 
      title 
      category 
      { categoryName } 
...
```graphql
query {
  booksByAuthor(nameOrId: "20") {
    title publicationYear language author { idAuthor }
  }
}
...
```graphql
query {
  booksByAuthor(nameOrId: "Author 20") {
    title publicationYear language author { name }
  }
}
...
```graphql
query {
  search(keyword: "book 2", type: BOOK) {
    list { 
      ... on Book 
      { title 
      author { name } 
...
```graphql
query {
  search(keyword: "Author 5", type: AUTHOR) {
    list {
       ... on Author 
          { name }
        }
...
```graphql
query {
  search(keyword: "category 2", type: CATEGORY) {
    list { 
      ... on Category {
         categoryName 
         } }
...
mutation {
  addBook(book: {
    ...
    idBook title author { name } category { categoryName }
  }
}
